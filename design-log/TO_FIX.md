...
- [ ] **Single-SKU pull (Holded→WC) doesn't update price — NOT reproduced; investigate secondary lead (page-load hang)** — Reported 2026-08-05: user changed price of SKU `SAN-TI-CUOOR-22-B-75-6` in Holded (29,50→29€), ran "Sync single SKU → Import from Holded" on production (poletto.es), price stayed 29,50€ in WC. Also could not load `wp-admin/admin.php?page=cthls-settings` at first to check the log.
  ...
  - **Separately confirmed via code reading (still valid, not the root cause of this report but a real gap):** in `includes/class-cthls-sync.php`, the price-update path for a **simple product** pull (~line 718-724) never calls `self::log()`, unlike the **variation** path (~line 653-668) which logs a `pull_price_check` event every time. This makes diagnosing a future simple-product price-sync issue much harder (no log trail at all). Confirmed still applicable after the version realignment to 1.6.5 (code was unchanged, only version metadata — see `DONE.md`).
